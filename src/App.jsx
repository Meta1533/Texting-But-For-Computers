...
function Chat({ user }) {
  ...
  const [userTag, setUserTag] =
    useState("just_chatting");
  const [showTagPicker, setShowTagPicker] =
    useState(false);
  const [profileTags, setProfileTags] = useState({});

  // Sidebar collapsible sections
  const [profileOpen, setProfileOpen] = useState(false);
  const [usernameEditorOpen, setUsernameEditorOpen] =
    useState(false);
  const [settingsOpen, setSettingsOpen] =
    useState(false);
  const [addContactOpen, setAddContactOpen] =
    useState(false);
  const [contactsOpen, setContactsOpen] =
    useState(false);
  const [groupsOpen, setGroupsOpen] =
    useState(false);
  const [groupMembersOpen, setGroupMembersOpen] =
    useState(false);
    ...
    function updatePresenceState() {
      ...
      Object.values(state).forEach((presences) => {
        ...
        users[presence.userId] = {
          ...
          username:
            presence.username || "Unknown user",
  ...
  async function changeTag(newTag) {
    ...
    if (error) {
      ...
      alert(
        "Could not save tag: " + error.message
      );
      return;
    }
  ...
  useEffect(() => {
    async function loadUnreadGroupMessages() {
      ...
      if (error) {
        console.error(
          "Error loading unread group messages:",
          error
        );
        return;
      }
      ...
      (data || []).forEach((msg) => {
        const storageKey =
          `group-last-read-${user.id}-${msg.group_id}`;
  ...
  useEffect(() => {
    async function loadConversation() {
      ...
      if (error) {
        console.error(
          "Error loading private messages:",
          error
        );
        setMessages([]);
        return;
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(
        `messages-${user.id}-${selectedContact.id}`
      )
      .on(
        "postgres_changes",
        {
  ...
  async function sendMessage() {
    ...
    const containsBannedWord = BANNED_WORDS.some(
      (word) =>
        trimmedMessage
          .toLowerCase()
    ...
    if (!selectedContact) {
      alert(
        "Select a contact before sending a message."
      );
      return;
    }
  ...
  async function removeContact(contactId) {
    ...
    if (remainingContact) {
      alert(
        "The contact is still in the database."
      );
      return;
    }
  ...
  async function createGroup() {
    ...
    if (trimmedName.length > 30) {
      setGroupError(
        "Group name must be 30 characters or less."
      );
      return;
    }
        ...
        >
          {contacts.length === 0 ? (
            ...
            contacts.map((contact) => {
              ...
              const tagId =
                profileTags[contactId];

              const tag =
                USER_TAGS[tagId];

              const presence =
                presenceUsers[contactId];
